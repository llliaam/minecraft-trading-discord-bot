...
import com.smp.marketplace.net.NotificationHandler;
import com.smp.marketplace.net.WsClient;
...
public class SmpMarketMod implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        ServerLifecycleEvents.SERVER_STARTED.register(server -> {
            EscrowLedger.get().load(server);
            // WS push notif (Fase F): mulai setelah server siap (butuh server ref).
            wsClient.setOnEvent(new NotificationHandler(server)::onEvent);
            wsClient.start();
        });
        ServerLifecycleEvents.SERVER_STOPPING.register(server -> {
            EscrowLedger.get().shutdown();
            wsClient.shutdown();
        });
        ...
        LOGGER.info(
            "SMP Market mod ter-inisialisasi. Command /link /market /marketbuy "
                ...
                + "(/marketbuy & /marketsell via GUI; offer via GUI /market; WS push notif aktif).");
    }
}
